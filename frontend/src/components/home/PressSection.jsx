      ...
      <div className="c">
        ...
        <div className="grid md:grid-cols-3 gap-5 mb-14">
          {PRESS_ITEMS.map((p, i) => (
            <div
              key={i}
              className="reveal bg-slate-50 dark:bg-white/[0.03] border border-slate-100 dark:border-white/[0.07] rounded-2xl overflow-hidden flex flex-col gap-4 hover:border-slate-200 dark:hover:border-white/15 hover:shadow-lg transition-all duration-300 hover:-translate-y-1 cursor-pointer group"
            ...
            >
              ...
              <h3 className="font-display text-navy dark:text-white text-base leading-snug group-hover:text-gold dark:group-hover:text-gold transition-colors px-6">{p.title}</h3>

              <p className="text-slate-500 dark:text-white/40 text-sm leading-relaxed flex-1 px-6">{p.excerpt}</p>

              <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-white/[0.06] px-6 pb-6">
